...
RC IndexManager::insertEntry(IXFileHandle &ixFileHandle, const Attribute &attribute, const void *key, const RID &rid) {
    ...
    if (ixFileHandle.fileHandle.getNumberOfPages() < 1) {
        ...
        Node root = Node(attribute.type, page, &ixFileHandle);
        root.pageNum = 0;
        free(page);
        if (root.nodeType == RootLeaf) {
            ...
            int pos;
            bool exist;
            leaf->locateChildPos(pos, exist, keyValue);
            if (!exist) {
                leaf->insertKey(pos, keyValue);
...
RC IndexManager::split(vector<Node *> &route, IXFileHandle &ixFileHandle) {
    ...
    if (node->nodeType == Leaf) {
        ...
        if (!exist) {
            ...
            parentNode->insertChild(pos + 1, newLeaf.pageNum);
        } else {
            // pos + 1
            parentNode->insertChild(pos, newLeaf.pageNum);
        }
        ...
        if (!exist) {
            ...
            parentNode->insertChild(pos + 1, newInter.pageNum);
        } else {
            parentNode->insertChild(pos, newInter.pageNum);
        }
...
RC IndexManager::deleteEntry(IXFileHandle &ixFileHandle, const Attribute &attribute, const void *key, const RID &rid) {
    ...
    if (node.nodeType == RootLeaf) {
        int pos;
        bool exist;
        node.locateChildPos(pos, exist, keyValue);
        if (!exist) {
...
RC IndexManager::constructRouteToLeaf(IXFileHandle &ixFileHandle, vector<Node *> &route, Node *root, const void *key,
...
Node::Node(AttrType type, const void *page, IXFileHandle *ixfileHandle) {
    this->attrType = type;
    int offset = 0;
...
RC Node::serializeNode(IXFileHandle &ixfileHandle) {
    void *page = malloc(PAGE_SIZE);
    memset(page, 0, PAGE_SIZE);
    if ((this->nodeType == RootLeaf || this->nodeType == Leaf) && this->keys.size() == 1 &&
        ...
        for (int i = 0; i < this->overFlowPages.size(); i++) {
            if (i < this->overFlowPages.size() - 1) {
                int ridsSize = this->serializeOverflowPage(i * nRidInOP + nRidInNode, (i + 1) * nRidInOP + nRidInNode,
                                                           overflowPage);
                memcpy((char *) overflowPage + ridsSize, &this->overFlowPages[i + 1], sizeof(int));
            } else {
                int ridsSize = this->serializeOverflowPage(i * nRidInOP + nRidInNode, this->pointers[0].size(),
                                                           overflowPage);
                int nextPage = -1;
...
RC Node::insertKey(int &pos, AttrValue &keyValue) {
    if (this->keys.size() >= 1) {
        this->keys.insert(this->keys.begin() + pos, keyValue);
    } else {
        this->keys.emplace_back(keyValue);
    }
}
...
RC Node::locateChildPos(int &pos, bool &exist, AttrValue &keyValue) {
    int i;
    for (i = 0; i < this->keys.size(); i++) {
        if (AttrValue::compAttr(keyValue, this->keys[i], EQ_OP)) {
            pos = i + 1;
            exist = true;
            return 0;
        }
        if (AttrValue::compAttr(keyValue, this->keys[i], LT_OP)) {
            pos = i;
            exist = false;
            return 0;
        }
